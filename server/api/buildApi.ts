...
const attachEndpoints = (app: App) => {
  const routers = [thingRouter, projectRouter, internalRouter]
...
export const buildApp = (config: Config, services: Services): App => {
  ...
  app.use(async (ctx, next) => {
    ...
    logger.info(
      `${ctx.method} ${ctx.path} ${ctx.status} -> ${ctx.ip}:${ctx.request.socket.remotePort} (${duration}ms)`
    )
  })
  ...
  app.use(async (ctx, next) => {
    ...
    if (ctx.status === 404) {
      throw new NotFoundError({
        entity: 'endpoint',
        id: ctx.path,
        secondaryEntity: 'method',
        secondaryId: ctx.method,
      })
    }
  })
